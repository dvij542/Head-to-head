    ...
    public struct DiscreteKartState
    {
        ...
        **/
        public DiscreteKartState applyAction(DiscreteKartAction action, RacingEnvController environment, DiscreteGameParams gameParams)
        {
            ...
            int timeUpdate = 100;//(int)(computeTOC(kart, distanceInSection, radiusOfSection, newState.tireAge/10000f, getAverageVelocity(), newState.getAverageVelocity()) * gameParams.timePrecision);
            if (timeUpdate < 0)
            {
    ...
    public class DiscreteGameState
    {
        ...
        **/
        public List<DiscreteKartAction> nextMoves()
        {
            ...
            for (int i = (int)agent.m_Kart.GetMaxSpeed()-1; i < (int)agent.m_Kart.GetMaxSpeed(); i += gameParams.velocityBucketSize)
            {
                for (int j = 1; j < 5; j++)
                {
            ...
            //     Debug.Log("Denied due to max lane changes : " + envController.MaxLaneChanges);
            ...
            // {
                ...
                filteredActions = possibleActions.FindAll((action) =>
                {
                    DiscreteKartState appliedAction = currentState.applyAction(action, envController, gameParams);
                    ...
                    foreach (DiscreteKartState other in kartStates)
                    {
                        ...
                        if (!other.name.Equals(appliedAction.name) && other.section == appliedAction.section && appliedAction.lane == other.lane)// && Math.Abs(appliedAction.timeAtSection - other.timeAtSection) * 1.0f / gameParams.timePrecision < 0)
                        {
